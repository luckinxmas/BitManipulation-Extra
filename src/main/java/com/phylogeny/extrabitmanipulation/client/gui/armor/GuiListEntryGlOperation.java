...
public class GuiListEntryGlOperation<L> extends GuiListEntryChiseledArmor<GlOperation>
{
	...
	public void formatDataFields(int mouseX, int mouseY)
	{
		for (int i = 0; i < dataFields.size(); i++)
		{
			...
			if (!buttonPlus.isMouseOver() && !buttonMinus.isMouseOver() && (mouseX < field.x || mouseX > field.x + field.width
	...
	public void updateScreen(boolean isSelected)
	{
		...
		buttonPlus.y = buttonMinus.y = y - 2;
	}
	...
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks)
	{
		...
		for (GuiTextField field : dataFields)
		{
			GuiUtils.drawContinuousTexturedBox(GuiChiseledArmor.TEXTURE_GUI, field.x - 3,
					field.y - 2, 78, 115, field.width + 9, field.height + 3, 13, 12, 2, 100);
			field.drawTextBox();
		}
		buttonPlus.drawButton(mc, mouseX, mouseY, partialTicks);
		buttonMinus.drawButton(mc, mouseX, mouseY, partialTicks);
	}
